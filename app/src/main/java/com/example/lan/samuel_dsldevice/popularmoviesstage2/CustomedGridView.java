...
public class CustomedGridView extends GridView{
    ...
    int numOfCols; //user preferred number of columns

    boolean sizeCaptured;
    //private int selectedPosition;

    private MovieAdapter movieAdapter; //Adapter containing the data for this gridView instance

    public MoviesGridFragment movieFragment; //Reference of the container fragment for later callbacks

    public boolean triggerFragmentReset;
    ...
    public void onSizeChanged(int w, int h, int oldw, int oldh){
        ...
        { //Only if both the height and width are non-zero then we perform some work
            ...
            //if(gwPx != w) {
                ...
                //w / colMinWidth
                int colMinWidthDp = MovieUtility.getIntSharedPreference(getContext(),
                        ((Activity) getContext()).getResources().getString(R.string.min_img_dp_size_key), 0); //(int) getResources().getDimension(R.dimen.min_img_size);
                int coMinWidthPx = (int)MovieUtility.convertDpToPixel(colMinWidthDp, getContext());

                //If the min image width is greater than the available width, we adjust it
                if(coMinWidthPx > w)
                    coMinWidthPx = w;
            ...
            */
            ...
            if(triggerFragmentReset){

                ((PM2_MainActivity)movieFragment.getAttachedActivity()).resetFragment(movieFragment);
            }
            else {
                ...
                //    movieFragment.callFromOnsizeChangeOrSetAttributesOrOnLoadFinishedMethod = true;
                sizeCaptured = true;
    ...
    private void completeSetup(int w, int h){
        ...
        updateNumColumns(w,h);
    }

    private void adjustMetrics(int w, int h){
    ...
    public void updateNumColumns(int w, int h){

        if(w>0 && h>0)
            adjustMetrics(w, h);
        else if(w == -1 && h == -1)
        {
            int wd = MovieUtility.getIntSharedPreference(getContext(),((Activity) getContext()).getResources().getString(R.string.gridView_width_key), 0 );
            int ht = MovieUtility.getIntSharedPreference(getContext(),((Activity) getContext()).getResources().getString(R.string.gridView_height_key), 0 );
        ...
        else if(cCols == 0){ //When the number of columns is not specifically set, use the maximum number of columns
            /*
            numOfCols = maxCols / 2;
            if(numOfCols<3)
                numOfCols = 3;
    ...
    public synchronized boolean isSizeCaptured(){
